...
def swap_3opt(tour, a, c, e, opt_action):
    """
    Swaps two edges by reversing a section of nodes

    :param list tour: TSP tour
    :param int a: First index for the swap
    :param int c: Second index for the swap
    :param int e: Third index for the swap
    """
    assert 0 <= a and a < c and c < e and e < len(tour)
    ...
    if False:
        ...
        next_tours = []
        # next_tour = np.concatenate((p[:a+1], p[b:c+1]   , p[d:e+1]   , p[f:])) # identity
        next_tour = np.concatenate((p[:a+1], p[b:c+1]   , p[e:d-1:-1], p[f:])) # 2-opt  remove (c, c+1) (e, e+1)              add (c, e) (c+1, e+1)
        next_tours.append(next_tour)
        next_tour = np.concatenate((p[:a+1], p[c:b-1:-1], p[d:e+1]   , p[f:])) # 2-opt  remove (a, a+1) (c, c+1)              add (a, c) (a+1, c+1)
        next_tours.append(next_tour)

        next_tour = np.concatenate((p[:a+1], p[c:b-1:-1], p[e:d-1:-1], p[f:])) # 3-opt  remove (a, a+1) (c, c+1) (e, e+1)     add (a, c) (a+1, e) (c+1, e+1)
        next_tours.append(next_tour)
        next_tour = np.concatenate((p[:a+1], p[d:e+1]   , p[b:c+1]   , p[f:])) # 3-opt  remove (a, a+1) (c, c+1) (e, e+1)     add (a, c+1) (e, a+1) (c, e+1)
        next_tours.append(next_tour)
        next_tour = np.concatenate((p[:a+1], p[d:e+1]   , p[c:b-1:-1], p[f:])) # 3-opt  remove (a, a+1) (c, c+1) (e, e+1)     add (a, c+1) (e, c) (a+1, e+1)
        next_tours.append(next_tour)
        next_tour = np.concatenate((p[:a+1], p[e:d-1:-1], p[b:c+1]   , p[f:])) # 3-opt  remove (a, a+1) (c, c+1) (e, e+1)     add (a, e) (c+1, a+1) (c, e+1)
        next_tours.append(next_tour)


        next_tour = np.concatenate((p[:a+1], p[e:d-1:-1], p[c:b-1:-1], p[f:])) # 2-opt  remove (a, a+1) (e, e+1)              add (a, e) (a+1, e+1)
        next_tours.append(next_tour)

        tours_cost = [route_distance(t, D) for t in next_tours]

        best_tour_id = tours_cost.index(min(tours_cost))

        return next_tours[best_tour_id], tours_cost[best_tour_id]
    
    else:
        if opt_action == 0:
            next_tour = np.concatenate((p[:a+1], p[e:d-1:-1], p[b:c+1], p[f:])).astype(dtype=np.int64)
        elif opt_action == 1:
            next_tour = np.concatenate((p[:a+1], p[d:e+1], p[b:c+1], p[f:])).astype(dtype=np.int64)
        else:
            next_tour = np.concatenate((p[:a+1], p[c:b-1:-1], p[d:e+1], p[f:])).astype(dtype=np.int64)

        next_tour = [int(i) for i in next_tour]
        

        return next_tour

def swap_3opt_dsz(tour, a, c, e, tour_distance, D, opt_action):
    """
    Swaps two edges by reversing a section of nodes
    ...
    if False:
        ...
        next_cost = route_distance(next_tour, D)

        return next_tour, next_cost
