...
parser.add_argument('--n_points',
                    type=int, default=100, help='Number of points in the TSP')
...
parser.add_argument('--gpu_n',
                    default=2, type=int, help='Choose GPU')
# --------------------------------- Network --------------------------------- #
...
class buffer:
    ...
    def clear_buffer(self):
        ...
        del self.opt_actions[:]
        del self.log_opt_probs[:]
        del self.opt_entropies[:]


def select_action(state, hidden, buffer, best_state, env):

    probs, action, log_probs_action, v, entropy, hidden,  \
    _, opt_action, log_opt_action_probs, opt_entropy = policy(
        state,
        best_state,
        hidden,
        env=env
    ...
    # one_mask_0 = torch.ones_like(opt_action)
    # one_mask_1 = torch.ones_like(opt_action)
    # one_mask_2 = torch.ones_like(opt_action)
    # one_mask_2[opt_action == 2] = 0.0

    # masks = torch.stack((one_mask_0, one_mask_1, one_mask_2)).transpose(1, 0)

    # log_probs_action = log_probs_action * masks
    ...
    buffer.opt_actions.append(opt_action)
    buffer.log_opt_probs.append(log_opt_action_probs)
    buffer.opt_entropies.append(opt_entropy)

    return action, v, hidden, opt_action
...
def learn(R, t_s, beta, zeta, count_learn, epoch):
    ...
    log_probs_sum = torch.stack(buffer.log_probs) + torch.stack(buffer.log_opt_probs)   # / 3.0
    log_probs = log_probs_sum.mean(2).unsqueeze(2)

    # entropies = torch.stack(buffer.entropies).mean(2).unsqueeze(2)
    entropies_sum = torch.stack(buffer.entropies) + torch.stack(buffer.opt_entropies)   # / 3.0
    entropies = entropies_sum.mean(2).unsqueeze(2)
...
for epoch in range(args.epochs):
    ...
    for batch_idx, batch_sample in enumerate(train_loader):
        ...
        while t < args.n_steps:
            ...
            while t - t_s < args.n and t != args.n_steps:
                ...
                action, v, _, opt_action = select_action(state,
                                             hidden,
                                             buffer,
                                             best_state,
                                             env)
                ...
                next_state, reward, _, best_distance, _, next_best_state = \
                    env.step(action.cpu().numpy(), opt_action.cpu().numpy())
            ...
            if args.update_value:
                ...
                _, _, _, next_v, _, _,   _, _, _, _ = policy(next_state, next_best_state, hidden)
                R = next_v
            else:
                R = None
            count_learn += 1
    ...
    for val_batch_idx, val_batch_sample in enumerate(test_loader):
        ...
        while t < args.n_steps:
            ...
            best_state = torch.from_numpy(best_state).float().to(device)
            with torch.no_grad():
                probs, action, _, _, _, _,     _, opt_action, _, _ = policy(state, best_state, hidden, env=env)
            sum_probs += probs
    ...
    if val_rwd_log.exp_avg > best_running_reward \
       ...
       or (args.test_from_data and gap < best_gap):
        ...
        model_dir = os.path.join(args.model_dir, args.comment)
        ...
        torch.save(checkpoint, os.path.join(model_dir,
                                            'pg-{}-TSP{}-epoch-{}.pt'
                                            .format(args.comment, args.n_points,
                                                    epoch)))
