...
_epsilon_inv = 1.0 / _epsilon
_knn = KNN(k=_K, transpose_mode=True)
...
class Encoder(nn.Module):
    ...
    def forward(self, input, hidden=None, env=None, is_best=False):
        """
        ...
        batch_size, graph_size = input.size(0), input.size(1)
        ...
        if False:
            ...
            if env.KNN_BUFFER is None:
                knn_dist, knn_indx = _knn(input, input)
                knn_dist = _epsilon / (knn_dist + _epsilon)

                knn_dist_sum = knn_dist.sum(dim=2)
                knn_dist /= knn_dist_sum.unsqueeze(-1)

                knn_dist = knn_dist.unsqueeze(-1)
                knn_dist.requires_grad = False
            
                env.KNN_BUFFER = (knn_dist, knn_indx)



            else:

                ____knn_dist, ____knn_indx = env.get_knn(d=input.device, is_best=is_best)

                knn_dist = ____knn_dist
                knn_indx = ____knn_indx


            knn_indx = knn_indx.reshape((batch_size, -1))
            knn_indx_dummy = knn_indx.unsqueeze(2).expand(knn_indx.size(0), knn_indx.size(1), self.embedding_dim)

            def gcn_layer(embedding, knn_index, knn_dist, embed_f):
                B, A = knn_index, embed_f(embedding)
                out = torch.gather(A, 1, B)
                out = out.reshape((batch_size, graph_size, _K, -1))

                out *= knn_dist     # e_ij * W_l * h_j
                out = out.sum(dim=2)

                return embedding + F.relu(out)

            g_embedding = self.embedding(input)
            g_embedding = gcn_layer(g_embedding, knn_indx_dummy, knn_dist, self.g_embedding)
            g_embedding = gcn_layer(g_embedding, knn_indx_dummy, knn_dist, self.g_embedding1)
            g_embedding = gcn_layer(g_embedding, knn_indx_dummy, knn_dist, self.g_embedding2)
    ...
    def forward(self, q, ref, inp, actions=None, g_emb=None, q_star=None):
        ...
        for i in range(self.n_actions):
            ...
            if actions is None:
                ...
                # indices = actions[:, i]
                indices = masked_prob.argmax(axis=1)
                log_probs_idx = c.log_prob(indices)
                dist_entropy = c.entropy()
...
class ActorCriticNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder_opts = nn.Sequential(
                                # nn.Linear(hidden_dim, hidden_dim),
                                # nn.ReLU(),
                                nn.Linear(hidden_dim, opts_num),
                                nn.Softmax(dim=1)
                            )
        ...
        self.film_generator = nn.Sequential(
            ...
            nn.Linear(self.embedding_dim, self.n_layers * self.hidden_dim * 2)
        )

        # OUTPUT
        self.activation = torch.nn.LeakyReLU()        
        self.network = nn.ModuleList([
            MLP(self.embedding_dim, self.hidden_dim, self.activation),
            MLP(self.hidden_dim, self.hidden_dim, self.activation),
            MLP(self.hidden_dim, self.hidden_dim, self.activation)
        ])


        self.W_r = nn.Linear(hidden_dim*3, hidden_dim)

    def forward(self, inputs, inputs_star, hidden=None, actions=None, env=None):
        # t_start = time.time()
        # print(env)

        # print("encoder_star")
        _, s_hidden_star, _, _ = self.encoder_star(inputs_star, hidden, env, is_best=True)

        # print("encoder")
        s_out, s_hidden, _, g_embedding = self.encoder(inputs, hidden, env, is_best=False)
        ...
        ### FILM
        pts_idx = pts.unsqueeze(2).expand(pts.size(0),pts.size(1),g_embedding.size(2))         
        pts_embedding = torch.gather(g_embedding, 1, pts_idx)
        pts_embedding = pts_embedding.sum(dim=1)


        s_out_mean = torch.mean(s_out, dim=1)

        film_parameters = self.film_generator(pts_embedding)
        film_parameters = film_parameters.view(-1, self.n_layers, 2, self.hidden_dim)

        x = s_out_mean
        for n, subnet in enumerate(self.network):
            x = subnet(x, film_parameters[:, n])

        opt_probs = self.decoder_opts(x)
        # t_decoder_opts = time.time()

        # print(opt_probs.shape)
        c = torch.distributions.Categorical(opt_probs)

        if actions is None:
            opt_indices = c.sample()
            log_opt_probs_idx = c.log_prob(opt_indices)
            opt_dist_entropy = c.entropy()
        else:
            opt_indices = opt_probs.argmax(axis=1)
            log_opt_probs_idx = c.log_prob(opt_indices)
            opt_dist_entropy = c.entropy()

        return probs, pts, log_probs_pts, v, entropies, enc_h, \
                opt_probs, opt_indices, log_opt_probs_idx[:,None].expand_as(log_probs_pts) , opt_dist_entropy[:, None].expand_as(entropies)
