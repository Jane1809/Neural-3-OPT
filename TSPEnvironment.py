...
class TSPInstanceEnv():
    ...
    def reset(self, points, tour, T=None):
        ...
        self.keep_tour_inv = self.inv_perm(self.keep_tour)
        ...
        self.current_best_tour_inv = self.inv_perm(self.current_best_tour)
    ...
    def step(self, action, opt_action):
        """
        Next observation of the TSP Environment
        ...
        reward = self._take_action(action, opt_action)
    ...
    def _take_action(self, action, opt_action):
        """
        ...
        self.tour = utils.swap_3opt(self.tour,
                                    action[0],
                                    action[1],
                                    action[2], opt_action)
        ...
        #                                                       self.tour_distance,
        #                                                       self.distances)
        self.new_keep_tour, self.new_tour_distance = utils.swap_3opt_dsz(self.keep_tour,
                                                              action[0],
                                                              action[1],
                                                              action[2],
                                                              self.tour_distance,
                                                              self.distances, opt_action)
        ...
        self.hist_current_distance.append(self.tour_distance)
        self.hist_best_distance.append(self.current_best_distance)
        # print("TETTETE", self.new_keep_tour)
        # input()
...
class VecEnv():
    ...
    def get_knn(self, d, is_best=False):

        knn_dist_init, knn_index_init = self.KNN_BUFFER
        # return knn_dist_init, knn_index_init
        knn_dist_init = knn_dist_init.squeeze()

        keep_tours = np.ndarray((self.n_envs, self.n_nodes), dtype=np.int64)
        keep_tours_inv = np.ndarray((self.n_envs, self.n_nodes), dtype=np.int64)

        for env_i, env in enumerate(self.envs):
            if is_best:
                x = np.array(env.current_best_tour, dtype=np.int64)
                y = np.array(env.current_best_tour_inv, dtype=np.int64)
            else:
                x = np.array(env.keep_tour, dtype=np.int64)
                y = np.array(env.keep_tour_inv, dtype=np.int64)
            keep_tours[env_i] = x
            keep_tours_inv[env_i] = y

        keep_tours_inv = torch.from_numpy(keep_tours_inv).to(d)
        keep_tours = torch.from_numpy(keep_tours).to(d).unsqueeze(-1)
        keep_tours = keep_tours.expand(knn_dist_init.shape)

        knn_dist = torch.gather(knn_dist_init, dim=1, index=keep_tours).unsqueeze(-1)
        knn_index_tmp = torch.gather(knn_index_init, dim=1, index=keep_tours)


        I = torch.arange(keep_tours_inv.size(0)).unsqueeze(-1).unsqueeze(-1)
        knn_index = keep_tours_inv[I, knn_index_tmp]

        # knn_index_tmp

        # print(knn_dist_init.shape)
        # print(knn_index_init.shape)
        # print(keep_tours.shape)
        # print(I.shape)
        # print(knn_index_tmp.shape)
        # print(keep_tours_inv.shape)
        # input()
    ...
    def step(self, actions, opt_actions):
        ...
        idx = 0
        for env in self.envs:
            obs, reward, done, best_obs = env.step(actions[idx], opt_actions[idx])
            # if idx == 0:
            #     print("tour:", " ".join([str(x) for x in env.keep_tour]))
            self.best_distances[idx] = env.current_best_distance
            self.distances[idx] = env.tour_distance
